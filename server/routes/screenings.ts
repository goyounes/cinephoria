...
router.get("/", verifyEmployeeJWT,
    ...
    async (req: Request, res: Response) => {
        const screenings = await getAllScreeningsAdmin(null, null);
...
router.get("/upcoming",
    ...
    async (req: Request, res: Response) => {
        const screenings = await getUpcomingScreenings(null, null);
        respondWithJson(res, screenings);
        saveToCache(req, screenings);
    });
...
router.get("/upcoming/:id",
    ...
    async (req: Request, res: Response) => {
        ...
        const rawscreenings = CombineGenresIdNames([rawwscreenings] as any)[0]; //cheated by submiting an array to the function and then taking the one elment out
        const screenings = CombineQualitiesIdNames([rawscreenings] as any)[0];   //cheated by submiting an array to the function and then taking the one elment out
        respondWithJson(res, screenings);
        saveToCache(req, screenings);
    });
...
router.get("/:id", verifyEmployeeJWT,
    ...
    async (req: Request, res: Response) => {
        ...
        const rawscreenings = CombineGenresIdNames([rawwscreenings] as any)[0]; //cheated by submiting an array to the function and then taking the one elment out
        const screenings = CombineQualitiesIdNames([rawscreenings] as any)[0];
