...
import { verifyEmployeeJWT, verifyUserJWT } from '../middleware/authMiddleware.js';
import { MovieWithGenresArray } from '../utils/CombineGenresIdNames.js';

// Extended interface for movies with S3 image URLs
interface MovieWithImage extends MovieWithGenresArray {
    imageUrl?: string;
}
...
import { getUpcomingScreenings, getUpcomingAndPastScreeningsAdmin } from '../controllers/screenings.js';
...
router.get("/",
    ...
    async (req: Request, res: Response) => {
        ...
        const movies: MovieWithImage[] = CombineGenresIdNames(rawMovies);
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const result = await addMovie({
                ...
                length: `${req.body.length_hours || "00"}:${req.body.length_minutes || "00"}:${req.body.length_seconds || "00"}` as any,
...
router.get("/upcoming",
    ...
    async (req: Request, res: Response) => {
        const rawMovies = await getUpcomingMoviesWithGenres(null);
        const movies: MovieWithImage[] = CombineGenresIdNames(rawMovies);
        for (const movie of movies) {
            const getObjectParams = {
                Bucket: bucketName,
...
router.get("/upcoming/all", verifyEmployeeJWT,
    ...
    async (req: Request, res: Response) => {
        const rawMovies = await getUpcomingMoviesWithGenresAdmin(null);
        const movies: MovieWithImage[] = CombineGenresIdNames(rawMovies);
        for (const movie of movies) {
            const getObjectParams = {
                Bucket: bucketName,
...
router.get("/:id",
    ...
    async (req: Request, res: Response) => {
        ...
        const movie: MovieWithImage = CombineGenresIdNames([rawMovie])[0]; //cheated by submiting an array to the function and then taking the one elment out
...
router.post("/reviews", verifyUserJWT, async (req: Request, res: Response) => {
