...
if (!apiKey) {
    throw new Error('Missing required SENDGRID_API_KEY environment variable');
}
...
export async function sendVerificationEmail(userEmail: string, verificationLink: string): Promise<any> {
    const msg = {
        to: userEmail,
        from: NO_REPLY_EMAIL,
...
export async function sendContactMessage({ name, email, subject, message }: ContactMessageParams): Promise<any> {
    const msg = {
        to: CONTACT_EMAIL,
...
export async function sendContactAcknowledgment({ name, email, subject, message }: ContactMessageParams): Promise<void> {
    const msg = {
        to: email,
        from: NO_REPLY_EMAIL,
...
export async function sendPasswordResetEmail(userEmail: string, resetLink: string): Promise<any> {
    const msg = {
        to: userEmail,
        from: NO_REPLY_EMAIL,
        subject: "Reset Your Cinephoria Password",
        html: `
            <h2>Password Reset Requested</h2>
            <p>Click the button below to reset your password. This link is valid for 1 hour:</p>
            <a href="${resetLink}" style="background:#dc3545;color:#fff;padding:10px 20px;text-decoration:none;">Reset Password</a>
            <p>If the button doesn't work, copy and paste this URL into your browser:</p>
            <p>${resetLink}</p>
        `,
