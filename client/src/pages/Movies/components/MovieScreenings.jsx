...
const MovieScreenings = ({ movieId }) => {
  ...
  useEffect(() => {
    ...
    const fetchScreenings = async () => {
      try {
        // Try employee screenings route first
        await axios.post("/api/auth/verify/employee");
        const { data } = await axios.get(`/api/movies/${movieId}/screenings/all`);
        setScreenings(data);
      } catch {
        try {
          // Fallback non-employee route
          const { data } = await axios.get(`/api/movies/${movieId}/screenings`);
          setScreenings(data);
        } catch (err) {
  ...
  const handleNext = async () => {
    ...
    setPage(newPage);
    setSelectedIndex(-1);
    if (isEmployee && !(await verifyEmployee())) navigate(0);
  };
